...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height
{
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, y + 5, (NSInteger)_chartMarginLeft * 0.9 + 10, (NSInteger)_yLabelHeight - 25)];
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleCircle) {
                ...
                CGPoint circleCenter = CGPointMake(circleRect.origin.x + (circleRect.size.width / 2), circleRect.origin.y + (circleRect.size.height / 2) - 8);
...
//********************** 线段开始点 + 结束点
                    [progressline moveToPoint:CGPointMake(last_x1, last_y1 - 10)];
                    [progressline addLineToPoint:CGPointMake(x1, y1 - 9)];
            ...
            // none style
                ...
                if ( i != 0 ) {
                    [progressline addLineToPoint:CGPointMake(x, y - 10)];
                    [lineStartEndPointsArray addObject:[NSValue valueWithCGPoint:CGPointMake(x, y)]];
                }
                
                [progressline moveToPoint:CGPointMake(x, y - 9)];
